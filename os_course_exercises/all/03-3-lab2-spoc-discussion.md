...
2. 不同特权级有什么区别？
> - 特权指令只能在ring 0使用
> - 同一指令在不同特权级下能访问的数据范围不同
> - 同一指令在不同特权级下行为不同

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
> 用于控制在不同特权级下所能进行的操作。详见我在piazza相关问题下的回答[关于x-86访问时特权级的问题](https://piazza.com/class/i5j09fnsl7k5x0?cid=1009)
...
> - 访问门时，需要判断CPL和RPL是否满足门本身的DPL限制
> - 访问代码段/数据段时，需要判断CPL和RPL是否满足段的DPL限制

2. 在什么情况下会出现特权级切换？
> - 以低特权级需要访问高特权级的nonconforming代码段时，需要切换特权级
> - 如在用户态发生中断/系统调用，需要执行核心态代码时，要切换特权级

3. int指令在ring0和ring3的执行行为有什么不同？
> - 在ring 0执行时不需要切换特权级，也不需要切换栈；在ring 3执行时需要切换特权级和相应的栈。
> - 在ring 0执行时，不会压入ss和esp寄存器的值

4. 如何利用int和iret指令完成不同特权级的切换？
> - 用int执行系统调用
> - 在系统调用内部，通过构造临时栈，求个·修改其中的cs, eip, eflags的值，切换特权级
> - 使用iret将系统调用栈内cs, eip, eflags的值写回相应寄存器，完成特权级切换并继续执行后续的代码
...
> - TSS用于保存ring 0, 1, 2的ss, esp等寄存器的值，也用来保存通用寄存器的值
> Task Register用来保存TSS的描述符，包括其基址、段限长等信息

### 7.3 了解段/页表

1. 一条指令执行时最多会出现多少次地址转换？
> 两次：逻辑地址->线性地址，线性地址->物理地址

2. 描述X86-32的MMU地址转换过程；
> 1. 取出逻辑地址中的页目录号，加上CR3寄存器中的页目录基址得到页目录项地址
> 2. 取出页目录项的值（20位），左移12位得到页表基址
> 3. 取出逻辑地址中的页表项号，加上页表基址得到页表项地址，取出页表项的值（20位），左移12位得到物理帧起始地址
> 4. 物理帧起始地址加上逻辑地址中的页内偏移offset，得到物理地址
...
> 1. 分配一页空间作为页目录表
> 2. 将页目录表清零
> 3. 将0xc0000000-0xf8000000和0x00000000-0x00100000都映射到物理地址的0x00000000-0x00100000
> 4. 将页目录信息写入cr3，并设置cr0的31 bit以打开页机制
> 5. 更新GDT
> 6. 取消0x00000000-0x00100000的映射
## 个人思考题
