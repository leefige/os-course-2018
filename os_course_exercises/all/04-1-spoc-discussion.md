...
- 存储器位置：寄存器、高速缓存片内，内存、外存片外
- 管理是否需要软件参与：cache不需要软件参与，其他需要
- 访问频率：频率依次降低

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
- 均衡繁忙：各存储介质都在繁忙状态，但不构成系统瓶颈
...
- 覆盖技术：将程序划分成功能模块，将不可能同时执行的模块分配到同一块内存
- 开发者：分析，划分模块，指定模块共享关系

2. 什么是交换技术？覆盖与交换有什么不同？
- 交换：进程作为一个整体从内存换入换出
- 与覆盖的不同：覆盖将程序划分为模块，交换以进程为单位

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
- 还有需要，但作为底层，不需要开发人员直接面对
...
- 增加以下标志位：
 ...
 2. 页式存储和虚拟页式存储的区别是什么？
 - 虚拟页式存储系统中，页表中部分页表项对应的页并不在物理内存中，而是被置换到了外存中

### 8.6 缺页异常

1. 缺页异常的处理流程？
  1. 指令执行中，根据页表项引用检查物理页是否在内存中；
  2. 由于页面不在内存，导致缺页异常；
  3. 在外存中查找需要访问的页面备份；
  4. 页面换入；
  5. 页表项修改；
